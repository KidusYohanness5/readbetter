...
5. Static UI lives in **`public/`** (`index.html`, `favicon.svg`) so the CDN serves the page and `app.py` handles `/api/*`. **`vercel.json`** bundles `public/**` into the Python function so `GET /` and `/favicon.svg` can still read those files when the request hits Starlette.
