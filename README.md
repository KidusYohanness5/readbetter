...
4. **Function duration:** In Vercel → Project → Settings → Functions, set **max duration** to **60s** (or as high as your plan allows) so PDF extraction + LLM can finish.
5. Static UI is in **`public/`** for the CDN. **Duplicate** `index.html` and `favicon.svg` also sit at the **repo root** next to `app.py` so the Python bundle on Vercel can read them (`public/` is not shipped into the serverless function). After editing `public/index.html` or `public/favicon.svg`, run: `cp public/index.html index.html && cp public/favicon.svg favicon.svg` (or keep them in sync some other way).
